...
contract OtherContract {
    ...
    function revertWith999() external pure returns (uint256) {
        assembly {
            mstore(0x00, 999)
            revert(0x00, 0x20) // sends the 999 message back
        }
    }
...
contract ExternalCalls {
    ...
    function externalViewCallNoArgs(address _a) external view returns (uint256) {
        assembly {
            ...
            //                                                         |       |
            //                                                         28      32
            let success := staticcall(gas(), _a, 28, 32, 0x00, 0x20) // has to be staticcal becaus eit is a view function
    ...
    function getViaRevert(address _a) external view returns (uint256) {
        assembly {
            ...
            pop(staticcall(gas(), _a, 28, 32, 0x00, 0x20)) // the result of staticcall is by default stored on the stack. Then popped
                // into a variable or nothing.
                // it captures the revert message 999
            return(0x00, 0x20)
        }
    }
    ...
    function callMultiply(address _a) external view returns (uint256 result) {
        assembly {
            ...
            // data is more than 64 bytes it cannot be stored in the scratch slot
            let success :=
                staticcall(
                    gas(),
                    _a,
                    add(oldMptr, 28),
                    mload(0x40), // should be 0x44 the size of the arguments
                    0x00,
                    0x20
                )
            if iszero(success) { revert(0, 0) }
        ...
        assembly {
            ...
            let success :=
                call(
                    ...
                    callvalue(),
                    28,
                    add(28, 32), // should be 4 + 32
                    0x00,
                    0x00
                )
            // If the function is not payable callvalue() will always be 0
            if iszero(success) { revert(0, 0) }
        }
    }
    ...
    function unknownReturnSize(address _a, uint256 amount) external view returns (bytes memory) {
        assembly {
            ...
            returndatacopy(0, 0, returndatasize()) // copy result of last call to
                // memory slot 0, copy return data from 0 to returndatasize of last context call
